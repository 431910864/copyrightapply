...
(function () {
  ...
  root.filterLocalStorageValue = function(value, valueKey, $this) {
    if (value) {
      value = {
        locacheTheValue: value,
        // locacheOpenId: $this.storage.get($this.key('openid')),
        timeOut: Number(new Date()),
      }
    }
    return value;
  };
  root.filterValue = function(value, valueKey, $this) {
    if (value && typeof value === 'object' && value.locacheTheValue) {
      // const openId = $this.storage.get($this.key('openid'));
      const { locacheOpenId, timeOut, locacheTheValue } = value;
      if (timeOut) {
        const time = (Number(new Date()) - value.timeOut) / 1000;
        // 过期时间3天
        if (time >= 60 * 60 * 24 * 3) {
      ...
      // if (locacheOpenId) {
      //   console.info(locacheOpenId, openId);
      //   if (locacheOpenId === openId) {
      //     return locacheTheValue;
      //   } else {
      //     return null;
      //   }
      // }
  ...
  LocacheCache.prototype.set = function (key, value, seconds) {
    ...
    value = root.filterLocalStorageValue(value, valueKey, this);
  ...
  LocacheCache.prototype.get = function (key) {
    ...
    if (value) {
      try {
        return root.filterValue(JSON.parse(value), valueKey, this);
      } catch (err) {
        return null;
      }
    }
